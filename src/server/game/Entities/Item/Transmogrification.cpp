...
void Transmogrification::LoadPlayerSets(Player* player)
{
   // TC_LOG_DEBUG("custom.transmog", "Transmogrification::LoadPlayerSets");
    ...
    QueryResult result = CharacterDatabase.PQuery("SELECT `PresetID`, `SetName`, `SetData` FROM `custom_transmogrification_sets` WHERE Owner = %u", player->GetGUID());
    if (!result)
        return;
    ...
    do
    {
        ...
        while (SetData.good())
        {
            ...
            if (slot >= EQUIPMENT_SLOT_END)
            {
				sLog->outDebug(LOG_FILTER_PLAYER_ITEMS, "custom.transmog", "Item entry (FakeEntry: %u, playerGUID: %u, slot: %u, presetId: %u) has invalid slot, ignoring.", entry, player->GetGUID(), uint32(slot), uint32(PresetID));
            ...
            if (sObjectMgr->GetItemTemplate(entry))
            {
				...
				sLog->outDebug(LOG_FILTER_PLAYER_ITEMS, "custom.transmog", "Item entry (FakeEntry: %u, playerGUID: %u, slot: %u, presetId: %u) does not exist, ignoring.", entry, player->GetGUID(), uint32(slot), uint32(PresetID));
        ...
        if (player->presetMap[PresetID].slotMap.empty())
        {
            ...
            CharacterDatabase.PExecute("DELETE FROM `custom_transmogrification_sets` WHERE Owner = %u AND PresetID = %u", player->GetGUID(), uint32(PresetID));
            return;
        }
...
TransmogTrinityStrings Transmogrification::Transmogrify(Player* player, uint64 itemGUID, uint8 slot, bool no_cost)
{
    ...
    if (slot >= EQUIPMENT_SLOT_END)
    {
		sLog->outDebug(LOG_FILTER_PLAYER_ITEMS, "custom.transmog", "Transmogrification::Transmogrify - %s (%u) tried to transmogrify an %u with a wrong slot (%u) when transmogrifying items.", player->GetName().c_str(), player->GetGUID(), itemGUID, slot);
    ...
    if (!itemGUID)
    {
        ...
        {
			sLog->outDebug(LOG_FILTER_PLAYER_ITEMS, "custom.transmog", "Transmogrification::Transmogrify - %s (%u) tried to transmogrify with an invalid %u.", player->GetName().c_str(), player->GetGUID(), itemGUID);
    ...
    {
		sLog->outDebug(LOG_FILTER_PLAYER_ITEMS, "custom.transmog", "Transmogrification::Transmogrify - %s (%u) tried to transmogrify an invalid item in a valid slot (slot: %u).", player->GetName().c_str(), player->GetGUID(), slot);
    ...
    else
    {
        ...
        {
			sLog->outDebug(LOG_FILTER_PLAYER_ITEMS, "custom.transmog", "Transmogrification::Transmogrify - %s (%u) failed CanTransmogrifyItemWithItem (%u with %u).", player->GetName().c_str(), player->GetGUID(), itemTransmogrified->GetEntry(), itemTransmogrifier->GetEntry());
        ...
        if (!no_cost)
        {
            ...
            {
                if (cost < 0)
					sLog->outDebug(LOG_FILTER_PLAYER_ITEMS, "custom.transmog", "Transmogrification::Transmogrify - %s (%u) transmogrification invalid cost (non negative, amount %i). Transmogrified %u with %u", player->GetName().c_str(), player->GetGUID(), -cost, itemTransmogrified->GetEntry(), itemTransmogrifier->GetEntry());
                else
                {
...
void Transmogrification::LoadConfig(bool reload)
{
   // TC_LOG_DEBUG("custom.transmog", "Transmogrification::LoadConfig");
    ...
    {
		sLog->outDebug(LOG_FILTER_PLAYER_ITEMS, "custom.transmog", "Transmogrification.TokenEntry (%u) does not exist. Using default (%u).", TokenEntry, 49426);
        TokenEntry = 49426;
    }
}
